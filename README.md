...
本项目目标是使用一个独立的 HTML 文件实现一个物理级高保真的 3x3 魔方交互演示。页面需基于 Three.js ES Modules 构建，支持自然手势拖拽、层旋转、视角旋转、打乱、重置、挑战计时与旋转音效功能。
...
页面提供 `Challenge` 按钮。点击后程序会先自动打乱魔方，打乱结束后立即开始计时。玩家手动复原魔方后，系统会根据贴纸当前朝向判断是否已归位，并自动停止计时、显示本次成绩。

最佳成绩使用 `localStorage` 保存在浏览器本地，刷新页面后仍可显示。`Reset` 会终止当前挑战并将本次计时归零。

### 旋转音效

页面提供 `Sound On / Sound Off` 开关。音效使用 Web Audio API 在浏览器内实时合成，不依赖额外音频文件。当前音效模拟魔方层旋转时的短促塑料卡扣声，自动打乱和玩家手动完成有效层旋转时都会播放。
...
- 松手后能磁吸到最近 90 度。
- 提供 Challenge、Scramble、Reset 和 Sound On / Sound Off 按钮。
- Challenge 会自动打乱魔方并开始计时。
- 玩家复原魔方后能自动停止计时并显示成绩。
- 最佳成绩能保存在浏览器本地。
- 旋转音效由 Web Audio API 实时生成，不依赖额外音频文件。
